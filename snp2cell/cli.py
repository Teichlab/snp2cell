...
import pickle
import logging
from typing import Union, Iterable, Literal, List, Optional
...
import snp2cell
from snp2cell import SNP2CELL, get_snp_scores, add_logger
...
def create_gene2pos_mapping(
    ...
    pos2gene_csv: Annotated[Path, typer.Argument(help="output path for csv file with mapping")] = "pos2gene.csv",
    host: Annotated[str, typer.Option("--biomart-host", "-h", help="biomart host to use; default: newest")] = 'http://www.ensembl.org',
    log = logging.getLogger(),
):
    """
    ...
    pd.DataFrame(pos2gene, index=[0]).T.sort_values(0).to_csv(pos2gene_csv, header=False)
...
def filter_summ_stats(
    s2c_obj: Annotated[Path, typer.Argument(help="path to SNP2CELL object")], 
    summ_stat_bed: Annotated[Path, typer.Argument(help="path to tsv file with summary statistics")],
    ld_score_bed: Annotated[Path, typer.Argument(help="path to tsv file with ld scores")],
    out_name: Annotated[Path, typer.Argument(help="output path for filtered summary statistics")],
    summ_stat_header: Annotated[str, typer.Option(help="comma separated string with header")] = None,
    ld_score_header: Annotated[str, typer.Option(help="comma separated string with header")] = None,
    pos2gene_csv: Annotated[Path, typer.Option("--pos2gene", "-p", help="csv file with no header and location (chrX:XXX-XXX) to gene symbol mapping; default: retrieve from biomart")] = None,
    log = logging.getLogger(),
):
    """
...
def score_snp(
    s2c_obj: Annotated[Path, typer.Argument(help="path to SNP2CELL object")], 
    summ_stat_bed: Annotated[Path, typer.Argument(help="path to tsv file with summary statistics")], 
    save_key: Annotated[str, typer.Option("--save-key", "-k", help="name for saving scores in object")] = "snp_score",
    pos2gene_csv: Annotated[Path, typer.Option("--pos2gene", "-p", help="csv file with no header and location (chrX:XXX-XXX) to gene symbol mapping; default: retrieve from biomart")] = None,
...
def contrast_scores(
    s2c_obj: Annotated[Path, typer.Argument(help="path to SNP2CELL object")],
    score_key1: Annotated[str, typer.Argument(help="key for scores stored in object (main)")], 
    score_key2: Annotated[str, typer.Argument(help="key for scores stored in object (reference)")], 
    save_key: Annotated[str, typer.Option("--save-key", "-k", help="name for saving scores in object; default: `(score_key1 - score_key2)`")] = None,
    n_cpu: Annotated[int, typer.Option(help="number of cpus to use")] = None,
    log = logging.getLogger(),
):
    """
...
def score_de(
    ...
    groupby: Annotated[str, typer.Argument(help="`ad.obs` column with annotation for computing differential expression")] = "annot",
    groups: Annotated[Optional[List[str]], typer.Option("--group", "-g", help="restrict to groups of `groupby`; may be set multiple times; default is to use all groups")] = None,
    reference: Annotated[str, typer.Option(help="reference group to compare against; default is to compare against the rest")] = "rest",
    method: Annotated[str, typer.Option(help="method for DE calculation")] = "wilcoxon",
    rank_by: Annotated[RankByChoice, typer.Option("--rank-by", "-r", help="rank DE scores by absolute value, up- or downregulation; default: upregulation")] = RankByChoice.up,
    snp_score_key: Annotated[str, typer.Option("--snp-score-key", "-k", help="key for accessing saved snp scores in object")] = "snp_score",
    n_cpu: Annotated[int, typer.Option("--n-cpu", "-c", help="number of cpus to use")] = None,
    log = logging.getLogger(),
):
    """
...
def combine_scores(
    s2c_obj: Annotated[Path, typer.Argument(help="path to SNP2CELL object")], 
    groupby: Annotated[str, typer.Argument(help="`ad.obs` column with annotation used for DE scores")] = "annot",
    snp_score_key: Annotated[str, typer.Argument(help="key for accessing saved snp scores in object")] = "snp_score",
    log = logging.getLogger(),
):
    """
